            ...
            <div style={{ marginTop: 28 }}>
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                ...
                <button
                  ...
                  disabled={tailorLoading || loading}
                  style={{
                    ...button,
                    padding: "12px 16px",
                    opacity: tailorLoading ? 0.6 : 1,
                  }}
                >
                  {tailorLoading
                    ? "Generating Agentic Resume..."
                    : "Generate Agentic Resume"}
                </button>
              </div>

              {tailoredResumeData && (
                <div style={{ marginTop: 24, padding: 16, background: "rgba(0,255,156,.05)", border: "1px solid rgba(0,255,156,.2)", borderRadius: 8 }}>
                  <div style={{ color: colors.neon, fontWeight: 700, marginBottom: 12, font: `12px ${mono}` }}>
                    ✅ Full Resume Generated!
                  </div>
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    {tailoredResumeData.pdf_file && (
                      <a
                        href={`${API}/download-resume/${tailoredResumeData.pdf_file.split('/').pop()}`}
                        target="_blank"
                        rel="noreferrer"
                        style={{ padding: "6px 12px", background: colors.neon, color: "#000", textDecoration: "none", borderRadius: 4, font: `11px ${mono}`, fontWeight: 600 }}
                      >
                        📥 Download PDF
                      </a>
                    )}
                    {tailoredResumeData.tex_file && (
                      <a
                        href={`${API}/download-resume/${tailoredResumeData.tex_file.split('/').pop()}`}
                        target="_blank"
                        rel="noreferrer"
                        style={{ padding: "6px 12px", border: `1px solid ${colors.neon}`, color: colors.neon, textDecoration: "none", borderRadius: 4, font: `11px ${mono}`, fontWeight: 600 }}
                      >
                        📥 Download .tex
                      </a>
                    )}
